...
export interface FromJSONDocSubscriberParams<B extends Bzz = Bzz> extends FromJSONDocParams<B> {
    pullInterval: number;
}
export interface LoadDocSubscriberParams<B extends Bzz = Bzz> extends LoadDocParams<B> {
    pullInterval: number;
}
...
export interface FromJSONDocSynchronizerParams<B extends Bzz = Bzz> extends FromJSONDocParams<B> {
    pullInterval: number;
    pushInterval?: number;
    sources?: Array<DocSerialized>;
}
export interface LoadDocSynchronizerParams<B extends Bzz = Bzz> extends LoadDocParams<B> {
    pullInterval: number;
    pushInterval?: number;
    sources?: Array<FeedParams>;
}
